            ...
            int mods)
{
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		g_camera.zoomKey(true);
	} else if (key == GLFW_KEY_S && action != GLFW_RELEASE) {
		g_camera.zoomKey(false);
	} else if (key == GLFW_KEY_A && action != GLFW_RELEASE) {
		g_camera.panHor(true);
	} else if (key == GLFW_KEY_D && action != GLFW_RELEASE) {
		g_camera.panHor(false);
	} else if (key == GLFW_KEY_LEFT && action != GLFW_RELEASE) {
		g_camera.roll(true);
	} else if (key == GLFW_KEY_RIGHT && action != GLFW_RELEASE) {
		g_camera.roll(false);
	} else if (key == GLFW_KEY_DOWN && action != GLFW_RELEASE) {
		g_camera.panVert(false);
	} else if (key == GLFW_KEY_UP && action != GLFW_RELEASE) {
		g_camera.panVert(true);
...
MousePosCallback(GLFWwindow* window, double mouse_x, double mouse_y)
{
	if (!g_mouse_pressed)
		return;
	//If the mouse was just clicked, need to update mouse position
	if(g_camera.getMouseMovement()) {
		g_camera.setMousePos(mouse_x, mouse_y);
		g_camera.setMouseMovement(false);
	}
	if (g_current_button == GLFW_MOUSE_BUTTON_LEFT) {
		g_camera.rotateMouse(mouse_x, mouse_y);
	} else if (g_current_button == GLFW_MOUSE_BUTTON_RIGHT) {
		g_camera.zoomMouse(mouse_y);
	}
}
...
{
	g_camera.setMouseMovement(true);
...
int main(int argc, char* argv[])
{
  ...
  std::vector<glm::vec4> floor_vtx_normals;
  std::vector<glm::uvec3> floor_faces;
	make_floor(floor_vertices, floor_vtx_normals, floor_faces);
